...
const upto = (str:string):string =>{
    return str.toUpperCase();
}
console.log(upto('ngoduy'));

//Function as types
type asType = (num1: number, num2: number) => number;
const addNumber: asType = (num1, num2) => {
    return num1 + num2;
};
...
function tong(...nums: number[]): number {
...
function Default(num1: number, num2: number = 0): number {
    return num1 + num2;
}
//Optional parameter
function getValueOrDefault(value: number = 0) {
    if (value !== undefined) {
        return value;
    } else {
        return 0;
...
//Rest parameter
function rest(...nums: number[]): number {
    return nums.reduce((total, num) => total + num, 0);
}
...
//*********never */
// Kiểu never không bao giờ trả về một giá trị hoặc kết thúc bình thường.
...
//never
function NEVER(errorMsg: string): never {
